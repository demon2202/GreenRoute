...
const TrophyIcon = ({ size = 48, color = '#fbbf24' }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color}
    strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ display: 'block', margin: '0 auto 0.75rem' }}>
...
/* ── Avatar Circle ── */
const LeaderboardAvatar = ({ name, image, size = 44, border = 'none' }) => {
...
const Leaderboard = ({ user }) => {
  ...
  const filtered = query.trim()
    ? list.filter(u => u.displayName.toLowerCase().includes(query.toLowerCase()))
    : list;

  // Podium calculations (Top 3)
  const first = list.find(u => u.rank === 1);
  const second = list.find(u => u.rank === 2);
  const third = list.find(u => u.rank === 3);
  ...
  if (loading) {
    return (
      <div style={{ width: '100%', padding: '4rem 2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        ...
        <p style={{ marginTop: '1.25rem', color: 'var(--text-secondary, #64748b)', fontWeight: 600, fontSize: '0.95rem' }}>Loading Leaderboard Rankings…</p>
        <style>{`
  ...
  return (
    <div style={{ width: '100%', maxWidth: 1400, margin: '0 auto', fontFamily: 'inherit', paddingBottom: '3.5rem', paddingLeft: '2rem', paddingRight: '2rem' }}>
      ...
      <div style={{ marginBottom: '1.75rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={{ margin: 0, color: 'var(--text-secondary, #64748b)', fontSize: '0.92rem' }}>
            Compete with travel champions and view cumulative eco impact.
          </p>
        </div>
        ...
        {currentUser && (
          <div style={{
            ...
            <LeaderboardAvatar name={currentUser.displayName} image={currentUser.image} size={38} border="1.5px solid var(--primary)" />
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                <span style={{ fontSize: '1.15rem', fontWeight: 900, color: 'var(--primary, #10b981)' }}>#{currentUser.rank}</span>
                <span style={{ fontSize: '0.88rem', fontWeight: 700, color: 'var(--text-primary)' }}>· {currentUser.totalCo2Saved} kg saved</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          {second && (
            <div style={{
              ...
              <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '0.82rem', fontWeight: 500 }}>{second.totalTrips} journeys</p>
              <div style={{
          ...
          {first && (
            <div style={{
              ...
              <div style={{ position: 'absolute', top: -20, left: '50%', transform: 'translateX(-50%)', animation: 'bounce 3s infinite' }}>
                <TrophyIcon size={44} />
              </div>
              ...
              <p style={{ margin: 0, color: 'var(--primary)', fontSize: '0.85rem', fontWeight: 700 }}>🏆 ECO CHAMPION · {first.totalTrips} trips</p>
              <div style={{
          ...
          {third && (
            <div style={{
              ...
              <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '0.82rem', fontWeight: 500 }}>{third.totalTrips} journeys</p>
              <div style={{
                marginTop: '1rem', background: 'var(--bg-primary, #f8fafc)',
                padding: '0.6rem', borderRadius: 12, fontWeight: 800,
                color: '#b45309', fontSize: '0.95rem',
              }}>
                🥉 {third.totalCo2Saved} kg CO₂
              </div>
            </div>
          )}
      ...
      <div style={{
        ...
        <input
          type="text"
          placeholder="Search leaderboards by name…"
          value={query}
      ...
      <div style={{
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: 500 }}>
            ...
            <tbody>
              {/* If query is active, show matching Top 3 ranks here as list items */}
              {query && filtered.map(u => (
                <RowItem key={u.userId} item={u} isCurrentUser={user.email === u.email} />
              ))}
              
              {/* Otherwise, show remaining ranks > 3 */}
              {!query && remainingList.map(u => (
                <RowItem key={u.userId} item={u} isCurrentUser={user.email === u.email} />
              ))}
...
/* ── Ranked Table Row Item ── */
const RowItem = ({ item, isCurrentUser }) => (
  <tr style={{
    borderBottom: '1px solid var(--border-color, #f1f5f9)',
    background: isCurrentUser ? 'linear-gradient(90deg, rgba(16,185,129,0.06) 0%, transparent 100%)' : 'transparent',
    transition: 'background var(--dur-fast)',
    boxShadow: isCurrentUser ? 'inset 2.5px 0 0 var(--primary)' : 'none',
  }}>
    {/* Rank */}
    <td style={S.td}>
      {item.rank <= 3 ? (
        ...
        <span style={{
          ...
          width: 32, display: 'inline-block', textAlign: 'center',
        }}>
          #{item.rank}
        </span>
      )}
    ...
    <td style={S.td}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <LeaderboardAvatar name={item.displayName} image={item.image} size={36} />
        <div>
          <span style={{
            fontSize: '0.9rem', fontWeight: isCurrentUser ? 800 : 700,
          ...
          {isCurrentUser && (
            <span style={{
              marginLeft: 8, fontSize: '0.65rem', background: 'rgba(16,185,129,0.18)',
              color: 'var(--primary)', padding: '1px 6px', borderRadius: 20, fontWeight: 800,
            }}>
              YOU
            </span>
          )}
